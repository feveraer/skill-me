...
function EditToolbar(props) {
  ...
  const handleClick = () => {
    ...
    setRows((oldSkills) => [
      ...oldSkills,
      { id, name: "", percentage: 0, isNew: true },
    ]);
...
function App() {
  ...
  useEffect(() => {
    ...
    let storedMaxScale = localStorage.getItem("maxScale");
    if (storedSkills !== null) {
      setRows(JSON.parse(storedSkills));
    }
    if (storedMaxScale !== null && storedMaxScale !== "undefined") {
      setMaxScale(JSON.parse(storedMaxScale));
    }
  }, []);
      ...
      <Row className="mt-2">
        <Col className="col-12 col-md-6">
          ...
          >
            ...
            />
            <div className="mt-4 text-muted small text-uppercase">
              Maximum scale: {maxScale}
            </div>
            <Slider
              aria-label="Maximum scale"
              onChange={(event, newValue) => setMaxScale(newValue)}
              onChangeCommitted={(event, newValue) => commitMaxScale(maxScale)}
              value={maxScale}
